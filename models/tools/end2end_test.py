...
def run_validate(args, val_dataloader, _gaze_network, criterion_mse, smpl,mesh_sampler):
    ...
    log_losses180 = [AverageMeter() for i in range(18)]
    ...
    with torch.no_grad():        
        for iteration, batch in enumerate(val_dataloader):
            ...
            theta_rad = np.arctan2(gaze_label[:,[2]], gaze_label[:,[0]])
            theta_deg = np.degrees(theta_rad)%360

            #yaw000 = (theta_deg >= 0) & (theta_deg <= 10)
            #if torch.any(yaw000):
            #    print(gaze_dir[yaw000.squeeze()])
            #    print(theta_deg)
            ...
            # 10度ごとの表示
            #for i in range(18):
            #    yaw = (theta_deg >= (360/18)*i) & (theta_deg < (360/18)*(i+1))
            #    if torch.any(yaw):
            #        print( (360/18)*i,gaze_dir[yaw.squeeze()])
            #        loss = criterion_mse(direction,gaze_dir).mean()
            #        log_losses180[i].update(loss.item(), yaw.numel())

            #count = 0
            #for dloss in log_losses180:
            #    print((count)*20,"",dloss.avg)
            #    count = count + 1
...
def main(args):
    ...
    test_dataloader = DataLoader(
        ...
        dset, batch_size=4, shuffle=False, num_workers=16, pin_memory=True
    )
